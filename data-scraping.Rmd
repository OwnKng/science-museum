...
```{r}
resp <- GET(
  "https://collection.sciencemuseumgroup.org.uk/search/objects/categories/space-technology?&page[number]=1&page[size]=100",
...
pages <- seq(1, last_page_number)
```

```{r}
item_urls <- tibble (
  url = map(pages, ~ str_interp("https://collection.sciencemuseumgroup.org.uk/search/objects/categories/space-technology?&page[number]=${.x}&page[size]=100"))
) %>% 
  unnest(url)
